...
Cargo, npm, Python, Maven, Gradle, CMake, Bundler, gemspec, Go, and Composer manifests in the
...
not cross the language crate seam. C, C++, Go, Java, JavaScript, JSX, PHP, Python, Rust,
...
Go package imports and PHP declared names cross the adapter seam as owned analysis
values. The project crate builds package-member and declaration indexes once per
graph and applies local Go and Composer metadata. A package import may yield
several file edges while counting as one resolved reference. Project metadata comes from the same
