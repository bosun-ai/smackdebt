...
Default and diff terminal reports SHALL show separate architecture sections
with coverage, architecture health, leading affected areas, and concise
findings or changes. Path selection SHALL retain incoming and outgoing facts
needed to explain the selected scope. JSON version 2 SHALL retain the complete
root graph.
