...
- [ ] 1.1 Add descriptive test value objects for generated repositories,
  commits, worktree edits, invocations, and complete process results.
- [ ] 1.2 Create unique temporary repositories with isolated Git configuration,
  fixed identities, fixed dates, explicit branches, and cleanup owned by the
  fixture lifetime.
- [ ] 1.3 Add fact manifests with hand-calculated values shared by focused
  assertions and public-output cases.

## 2. Unified repositories

- [ ] 2.1 Add `all-languages` nested-unit fixtures that prove exact source
  metrics for every supported language.
- [ ] 2.2 Add `static-architecture` fixtures for resolved, external, unresolved,
  ambiguous, file-cycle, package-cycle, degree, and instability facts.
- [ ] 2.3 Add `evolution` fixtures for churn, renames, binary changes, repeated
  coupling, mailmap normalization, concentration, and privacy.
- [ ] 2.4 Add `worktree-change` fixtures covering metric, dependency, cycle,
  rename, delete, untracked, and coupling-explanation outcomes.
- [ ] 2.5 Add `coverage-failures` fixtures for unsupported and failed source,
  incomplete history, invalid refs, and outside-Git behavior.

## 3. Real CLI process harness

- [ ] 3.1 Invoke the built CLI with explicit directory, arguments, environment,
  width, color, and workers and capture status, stdout, and stderr bytes.
- [ ] 3.2 Assert success, policy, partial-coverage, invocation-failure, and
  report-failure exit and stream contracts.
- [ ] 3.3 Keep the harness outside internal report construction and analysis
  APIs so every case crosses the public CLI seam.

## 4. Exact report evidence

- [ ] 4.1 Add committed terminal results for default, `--all`, selected file,
  selected directory, selected package, codebase, ref-diff, and worktree-diff
  flows.
- [ ] 4.2 Add terminal results at widths 120, 80, and 50 and prove forced-color
  output has the same visible text as color-disabled output.
- [ ] 4.3 Validate every JSON result against the committed version-2 schema,
  assert key semantic values, and compare exact bytes.
- [ ] 4.4 Prove one-worker and automatic-parallel terminal and JSON bytes are
  identical for the complete analysis families.
- [ ] 4.5 Add an explicit local golden-update command that selects cases and
  reports changed files; ensure ordinary tests and CI cannot update evidence.

## 5. Work and privacy evidence

- [ ] 5.1 Instrument inventory visits, source and object reads, Git processes,
  parser visits, algorithm passes, and renderer entry behind test-only hooks.
- [ ] 5.2 Prove terminal and JSON rendering do not discover, read, parse, start
  Git, or rerun analysis after receiving the report.
- [ ] 5.3 Prove generated fixture contributor names and addresses are absent from
  terminal, JSON, diagnostics, and golden files.
- [ ] 5.4 Add complete-flow allocation, peak-memory, source-read, object-read,
  Git-process, wall-time, and p95 measurements for generated mixed repositories.

## 6. Installation and documentation

- [ ] 6.1 Install locked local crates into a unique temporary prefix and run the
  installed CLI from a generated repository outside the workspace.
- [ ] 6.2 Prove installed help, version, terminal report, JSON report, status, and
  stream behavior.
- [ ] 6.3 Mark executable README console examples, run them against named public
  fixtures, and compare their documented behavior.
- [ ] 6.4 Update `ARCHITECTURE.md`, schema documentation, test documentation, and
  release instructions with the evidence layers and update workflow.

## 7. Release gate

- [ ] 7.1 Prove every requirement from the source engine, static architecture,
...
- [ ] 7.2 Run formatting, Clippy, workspace tests, strict OpenSpec validation,
