...
language-provided structural, nesting, alternative, labeled-jump, and boolean-run
costs through one shared algorithm. Nested rated units SHALL NOT contribute to
their parent's cognitive value. Recursion SHALL NOT contribute without a
separate reliable call graph.
...
for every language-provided independent decision event. Nested rated units SHALL
NOT contribute decisions to their parent.
...
identities, spans, cognitive values, cyclomatic values, logical-line values, and
parse status. Existing output SHALL NOT override these expected values when it
conflicts with the defined metric rules.
