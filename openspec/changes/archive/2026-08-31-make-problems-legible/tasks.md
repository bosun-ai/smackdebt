...
- [x] 7.1 Calibrate the thresholds against real repositories: run the built binary over a Vue application and over this workspace at repository, package, directory, and file scope, review the resulting card sets, and pay particular attention to the rated-unit arm of the `god_file` rule where single-file components inflate unit counts. — DONE: the release binary was run over this workspace (root, `crates/analysis`, `crates/output`) and over a Vue/Rust/Python/Ruby monorepo (root and its `bow/src/components` directory). The risky arm was the defect the reviewer predicted: counting every rated unit made `does too much` 9 of 24 root cards here and 23 of 24 there, and 13 of 24 in the component directory. The arm now counts units rated Watch or High, and the threshold is 6. Six was chosen over 8 because it is the value at which the pattern names a minority without falling silent: 6 leaves 1 of 24 root cards here, 12 of 24 at the monorepo root, and 1 of 24 in the component directory — that one being a 133-unit component with 2 High findings, 4 more Watch units, 16 imports, and a size finding, which is exactly what `does too much` should name. At 8 that component loses the name and is called `everything depends on this` instead, and the pattern disappears from the component directory altogether.
- [x] 7.2 Freeze the reviewed constants, updating the specs where a proposed value moved. — DONE as reviewed rather than frozen, by controller ruling. One value moved: the `god_file` concentration arm now reads `BROAD_DEBT_UNITS = 6` over units rated Watch or High, and the problem-clustering delta says so. `CONCENTRATED_HIGH_FINDINGS`, `GOD_FILE_FAN_OUT`, `HUB_DEGREE`, `HUB_MEDIAN_MULTIPLE`, `SCREEN_BUDGET`, and `PROBLEM_LADDER` were reviewed against the same runs and left where they are. Every constant keeps its `proposed value under review` marker deliberately, so a later change can move one without contradicting an accepted spec. The share sentence's numbers stay ungrouped: the bytes are analysis-owned and frozen, the schema pins a digit-only pattern, and grouping inside a sentence reads oddly beside the ungrouped counts line.
- [x] 7.3 Update the README: the problem section in the codebase example, the pattern list with its words and thresholds, the one-screen budget, the new meaning of `--top` and `--all`, the share sentence, and the statement that edges are JSON-only.
- [x] 7.4 Update `ARCHITECTURE.md` for the clustering step and the analysis-owned card order against the renderer's per-variant words.
- [ ] 7.5 Confirm the committed ratchet baseline is unchanged with the gate, note the stale performance `report_digest` values, tick every task, pass strict validation and the complete check, and archive this change. — PARTLY DONE: the gate is green and its three reviewed baseline entries are recorded in the proposal, the stale `report_digest` values are recorded there too, strict validation and `just check` pass; archiving waits for the whole-branch review and for 7.1 and 7.2.
