...
/// A history request measured in calendar days by Git.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
/// Counts processes started by an adapter. This is intentionally small so a
/// caller can assert that history and batch reads do not scale with file count.
...
impl GitRepository {
    ...
    /// Open a known repository path, validating that it is a worktree.
    ...
    pub fn process_stats(&self) -> ProcessStats {
        ProcessStats {
            git_processes: self.processes.load(Ordering::Relaxed),
        }
    }
    ...
    pub fn ref_exists(&self, reference: &str) -> Result<bool, GitError> {
    ...
    pub fn changed_paths(&self, base: &str) -> Result<Vec<ChangedPath>, GitError> {
        validate_ref(base)?;
        let output = self.run_raw(vec![
            OsString::from("diff"),
    ...
    pub fn history(&self, window: HistoryWindow) -> Result<Vec<FileActivity>, GitError> {
        let args = vec![
            OsString::from("log"),
            ...
            OsString::from(format!("--since={} days ago", window.days)),
            OsString::from("--"),
        ];
    ...
    pub fn batch_reader(&self, max_pending: usize) -> Result<BatchObjectReader, GitError> {
        BatchObjectReader::start(self.root.clone(), self.processes.clone(), max_pending)
    }
...
impl fmt::Debug for BatchObjectReader {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
...
impl BatchObjectReader {
    ...
    pub fn read_object(&mut self, object: &str) -> Result<Vec<u8>, GitError> {
...
impl Drop for BatchObjectReader {
    fn drop(&mut self) {
...
mod tests {
    ...
    fn history_uses_one_process_and_counts_touches() {
        ...
        let adapter = GitRepository::discover(&repo.path).unwrap();
        let before = adapter.process_stats().git_processes;
        let activity = adapter.history(HistoryWindow::days(3650)).unwrap();
        assert_eq!(adapter.process_stats().git_processes - before, 1);
        assert_eq!(
            activity
                .iter()
    ...
    fn batch_reader_reuses_one_process() {
        ...
        .unwrap();
        let mut reader = adapter.batch_reader(2).unwrap();
    ...
    fn batch_reader_reads_paths_with_spaces_and_rejects_line_breaks() {
        ...
        let adapter = GitRepository::discover(&repo.path).unwrap();
        let mut reader = adapter.batch_reader(1).unwrap();
        assert_eq!(
            reader
